...
class ConsultarAsociacionesUseCase:
    ...
    def listar_por_sensor(
        ...
        # el filtro solo se aplica a un rol sin alcance global; un sensor ajeno
        # responde 404, no 403, para no confirmar su existencia.
        if ids_fincas_permitidas is not None:
            ...
            if area is None or area.id_finca not in ids_fincas_permitidas:
                raise NotFoundError(
                    code="SENSOR_NO_ENCONTRADO",
                    message=f"No existe un sensor con ID {id_sensor}.",
                )
