...
class CuentasUsuarios(Base):
    ...
    es_token_usado: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default=text('false'), comment='Marca si token_activacion_actual (de RECUPERACION) ya fue consumido en un restablecimiento exitoso. Se mantiene el hash (no se limpia al usarlo) para poder distinguir "token ya utilizado" (409) de "token nunca existió" (401) en un reintento. Se resetea a false al emitir un token nuevo.')
