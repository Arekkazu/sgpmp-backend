# TC-M02-G19 rev2 RF-34. Segunda evaluacion TEST. No modifica historico.
# Password via SGPMP_TEST_PASSWORD o CONTRASENA.

$ErrorActionPreference = "Stop"
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$RepoRoot = (Resolve-Path (Join-Path $ScriptDir "..\..\..\..\..")).Path
$TestFile = Join-Path $ScriptDir "test_tc_m02_g19_rev2.py"
$ResultadosDir = Join-Path $ScriptDir "Resultados"
$HtmlPytest = Join-Path $ResultadosDir "TC-M02-G19-rev2-pytest.html"
$TxtLog = Join-Path $ResultadosDir "TC-M02-G19-rev2-pytest.txt"
