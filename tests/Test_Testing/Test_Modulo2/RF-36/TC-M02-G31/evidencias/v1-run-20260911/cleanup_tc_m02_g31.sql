...
-- 4. Estado del lote 466 (lote temporal de preflight para baja total)
SELECT a.id_activo_biologico, a.tipo, e.nombre AS estado,
       d.cantidad_inicial, d.cantidad_actual, d.peso_promedio, d.biomasa_total,
       a.descripcion
FROM modulo2.activos_biologicos a
JOIN modulo2.estados_activos_biologicos e ON a.id_estado = e.id_estado_activo_biologico
JOIN modulo2.detalles_activos_biologicos_poblacionales d ON a.id_activo_biologico = d.id_activo_biologico
WHERE a.id_activo_biologico = 466;

-- ============================================================
-- LOTES CONSUMIDOS POR PRUEBAS — NO REUTILIZAR
-- ============================================================

-- Lotes 469, 470, 471: fecha_creacion alterada por hack 
-- (sesión V3 contaminada, 2026-09-19). Marcar como quemados.
SELECT id_activo_biologico, fecha_creacion, created_at
FROM modulo2.activos_biologicos
WHERE id_activo_biologico IN (469, 470, 471);

-- Lote 472: creado en preflight limpio 2026-09-19, quedó ACTIVO 
-- porque el defecto FECHA_INVALIDA impidió su baja.
SELECT id_activo_biologico, fecha_creacion, created_at
FROM modulo2.activos_biologicos
WHERE id_activo_biologico = 472;

-- Evento id_eventos=374 sobre lote 296 (preflight TC-M02-196).
SELECT id_eventos, id_activo_biologico, fecha, descripcion
FROM modulo2.eventos_biologicos
WHERE id_eventos = 374;
