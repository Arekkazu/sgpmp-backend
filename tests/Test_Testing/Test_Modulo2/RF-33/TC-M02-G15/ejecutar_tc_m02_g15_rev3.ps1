# TC-M02-G15 rev3 — cierre de bloqueos 028/031. No modifica historico ni rev2.
# Password via SGPMP_TEST_PASSWORD, CONTRASENA, SGPMP_INGENIERO_PASSWORD o ENGINEER_PASSWORD.

$ErrorActionPreference = "Stop"
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$RepoRoot = (Resolve-Path (Join-Path $ScriptDir "..\..\..\..\..")).Path
$TestFile = Join-Path $ScriptDir "test_tc_m02_g15_rev3.py"
$ResultadosDir = Join-Path $ScriptDir "Resultados"
$HtmlPytest = Join-Path $ResultadosDir "TC-M02-G15-rev3-pytest.html"
$TxtLog = Join-Path $ResultadosDir "TC-M02-G15-rev3-pytest.txt"
...
function Protect-Evidence([string]$Path) {
    ...
    foreach ($name in @('SGPMP_TEST_PASSWORD','CONTRASENA','SGPMP_INGENIERO_PASSWORD','ENGINEER_PASSWORD','SGPMP_VETERINARIO_PASSWORD','VET_PASSWORD')) {
