"""TC-M02-G16 rev3 — TC-M02-030 XSS/inyeccion en atributos_dinamicos.

No modifica rev1/rev2 ni codigo productivo. Sin DML. Sin crear metricas RF-16.
...
ENV_PATH = DIR / "environment-g16-rev3.json"
...
CTRL_TXT = "valor_control_qa_g16r3"
INJ_TXT = "x'; SELECT 1--"
INJ_JSON_STR = '{"$gt":""}'
...
EVIDENCIA: dict = {
    ...
    "auditoria": "Rev2 intacta. Sin DML. Sin POST metricas. Sin cambios productivos. Sin commit/push.",
}
...
def _escribir():
    ...
    (RESULTADOS / "TC-M02-G16-rev3.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2, default=str), encoding="utf-8"
    )
    subs = payload.get("subcasos") or {}
    lineas = [
        "TC-M02-G16 rev3 — RF-33 / CU01 — TEST — solo 030",
    ...
    (RESULTADOS / "TC-M02-G16-rev3.txt").write_text("\n".join(lineas) + "\n", encoding="utf-8")
    v = subs.get("TC-M02-030") or {}
    (RESULTADOS / "TC-M02-G16-rev3-evidencia.html").write_text(
        "<!DOCTYPE html><html lang='es'><head><meta charset='utf-8'>"
        "<title>TC-M02-G16 rev3</title></head><body>"
        "<h1>TC-M02-G16 rev3 — TC-M02-030</h1>"
...
def sesion():
    ...
    for aid in list(EVIDENCIA.get("ids_creados") or []):
        if tok_p:
            _restaurar(tok_p, int(aid), "Restauracion residual TC-M02-G16 rev3")
    _escribir()
...
def test_tc_m02_030_xss_atributos(sesion):
    ...
    for e in especies:
        ...
        if e.get("es_activo") is not False:
            activas.add(eid)
    if ESPECIE_PREF not in ids_esp:
        ids_esp.insert(0, ESPECIE_PREF)
    else:
        ids_esp = [ESPECIE_PREF] + [i for i in ids_esp if i != ESPECIE_PREF]
    ...
    control["restauracion"] = _restaurar(tok_p, aid_c, "Restauracion control TC-M02-G16 rev3")
    ...
    def _eval_post(etiqueta: str, valor, ident: str):
        ...
        if cap["http"] == 201 and cap["id_creado"]:
            ...
            cap["restauracion"] = _restaurar(tok_p, aid, f"Restauracion 030 {etiqueta} TC-M02-G16 rev3")
            EVIDENCIA["ids_creados"] = [x for x in EVIDENCIA["ids_creados"] if x != aid]
        out["intentos"].append(cap)
        return cap

    cap_xss = _eval_post("xss_script", XSS, f"QAG16R3X{STAMP}"[:50])
    cap_sql = _eval_post("inyeccion_texto", INJ_TXT, f"QAG16R3S{STAMP}"[:50])
    cap_json = _eval_post("operador_json_string", INJ_JSON_STR, f"QAG16R3J{STAMP}"[:50])
    ...
    if cap_obj["http"] == 201 and cap_obj["id_creado"]:
        ...
        EVIDENCIA["ids_creados"].append(aid)
        cap_obj["restauracion"] = _restaurar(tok_p, aid, "Restauracion 030 objeto TC-M02-G16 rev3")
        EVIDENCIA["ids_creados"] = [x for x in EVIDENCIA["ids_creados"] if x != aid]
