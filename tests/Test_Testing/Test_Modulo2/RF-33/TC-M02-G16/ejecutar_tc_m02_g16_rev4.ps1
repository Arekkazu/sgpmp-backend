# TC-M02-G16 rev4 — solo 030. No modifica rev2/rev3 ni backend.
$ErrorActionPreference = "Stop"
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$RepoRoot = (Resolve-Path (Join-Path $ScriptDir "..\..\..\..\..")).Path
$TestFile = Join-Path $ScriptDir "test_tc_m02_g16_rev4.py"
$ResultadosDir = Join-Path $ScriptDir "Resultados"
$HtmlPytest = Join-Path $ResultadosDir "TC-M02-G16-rev4-pytest.html"
$TxtLog = Join-Path $ResultadosDir "TC-M02-G16-rev4-pytest.txt"
$TxtNewman = Join-Path $ResultadosDir "TC-M02-G16-rev4-Newman.txt"
...
TC-M02-G16 rev4: Newman no se ejecuta contra TEST.
Motivo: {{contrasena}} vacia en coleccion provoca 400 (cwd leak). Veredicto = pytest.
"@ -Encoding UTF8
