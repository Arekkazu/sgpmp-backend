"""TC-M02-G97 V2 (reevaluacion) / TC-M02-314 - RF-51: falta de autorizacion de lectura.
...
Ejecucion:
    .venv\\Scripts\\python.exe -m pytest tests/Test_Testing/Test_Modulo2/RF-51/TC-M02-G97/EvaluacionV2/test_tc_m02_g97_v2.py \\
        -v --junitxml=EvaluacionV2/Resultados/reporte_tc_m02_g97_v2_pytest.xml
"""
...
def test_el_endpoint_de_indicadores_exige_read_sobre_activos_biologicos():
    """Verifica estáticamente que RF-51 exige READ (recurso 29, acción 2).

    Se lee el archivo fuente directamente para evitar importar el router
    completo y depender innecesariamente de DATABASE_URL.
    """
    ...
    for padre in archivo_actual.parents:
        candidato = (
            ...
            / 'infrastructure'
            / 'routers'
            / 'activo_biologico_router.py'
        )

        if candidato.exists():
            ruta_router = candidato
            break

    assert ruta_router is not None, (
        'No se encontró '
        'src/biological_assets/infrastructure/routers/'
        'activo_biologico_router.py'
    )

    fuente = ruta_router.read_text(encoding='utf-8')

    assert '_RECURSO = 29' in fuente, (
        'El recurso de activos biologicos cambió; revisar TC-M02-314'
    )

    marcador_ruta = "'/{id_activo}/indicadores'"
    marcador_funcion = 'def consultar_indicadores'

    assert marcador_ruta in fuente, (
        'No se encontró la ruta RF-51 /{id_activo}/indicadores'
    )

    bloque = fuente.split(marcador_ruta, 1)[1]

    assert marcador_funcion in bloque, (
        'No se encontró consultar_indicadores después de la ruta RF-51'
    )

    bloque = bloque.split(marcador_funcion, 1)[0]

    assert "require_permission_m02(_RECURSO, 2, rf_origen='RF51')" in bloque, (
        'El endpoint de indicadores ya no exige READ '
        '(recurso 29, acción 2); revisar TC-M02-314'
    )
