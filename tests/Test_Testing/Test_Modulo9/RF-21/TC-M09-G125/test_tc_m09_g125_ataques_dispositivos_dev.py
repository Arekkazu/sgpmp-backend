"""
TC-M09-G125 -- SEGUNDA CORRIDA CONTRA DEV (TC-M09-244, TC-M09-245,
TC-M09-246) - Pruebas de ataque: suplantacion y enumeracion de
dispositivos IoT (OWASP API1 Broken Object Level Authorization, API4
Unrestricted Resource Consumption, API8 Security Misconfiguration).

RF relacionado: RF-21, CU-05 Gestionar Dispositivos IoT
Categoria: Seguridad - Ataque (OWASP). Caso nuevo agregado por QA.

Re-ejecucion pedida por el usuario (2026-09-13) para verificar si las
correcciones del equipo de desarrollo (ver ramas fix/inc-m02-* en origin)
ya se ven reflejadas en el entorno DEV. La corrida original (2026-09-05,
test_tc_m09_g125_ataques_dispositivos.py, este mismo directorio) se deja
intacta como evidencia historica contra TEST; este archivo es una copia
apuntando a DEV, no un reemplazo.
...
en DEV (no mocks, no TestClient local) -- es la forma mas fiel de simular
...
compartida de DEV); todos los dispositivos creados por esta suite se
...
Como correrlo (desde la raiz del repo):
    python -m pytest <ruta>\\test_tc_m09_g125_ataques_dispositivos_dev.py -v \
        --html=Resultados/reporte-TC-M09-G125-DEV.html --self-contained-html

RESULTADO (2026-09-13): **3/3 PASARON en la corrida limpia** (ver
reporte-TC-M09-G125-DEV.html) -- los 3 hallazgos originales de TEST
(2026-09-05) parecen corregidos en DEV:
  - Escenario 1 (suplantacion via serial): ya funcionaba bien en TEST
    tambien (409 SERIAL_DUPLICADO), sigue funcionando en DEV.
  - Escenario 2 (SQLi/XSS sin validar): en TEST el payload se aceptaba
    con 201. En DEV, el MISMO payload fue rechazado con 400 -- el
    backend ahora SI valida el charset de serial/descripcion. Confirmado
    en la corrida limpia (primera peticion de la suite, sin contaminacion
    de rate limit).
  - Escenario 3 (sin rate limiting): en TEST las 50 peticiones en rafaga
    se aceptaban todas (0 bloqueadas). En DEV aparecio un nuevo codigo
    `429 LIMITE_TASA_EXCEDIDO` que antes no existia en absoluto en el
    codebase -- confirmado independientemente en 3 formas distintas: (a)
    el assert de la suite paso, (b) una corrida INMEDIATAMENTE posterior
    de la misma suite fallo en escenarios 1 y 2 porque el rate limit de
    la corrida anterior seguia activo (ambos recibieron 429 en vez de sus
    codigos normales), y (c) una peticion manual de verificacion via curl
    tambien recibio 429 mientras la ventana seguia activa. El rate
    limiting es real y esta activo en DEV.

CUIDADO al re-correr esta suite en el corto plazo: como ahora SI hay rate
limiting, correr esta suite dos veces seguidas (o junto con otras
pruebas que pegan al mismo endpoint) puede hacer que escenario 1/2
fallen por 429 en vez de por su logica real -- eso es un FALSO POSITIVO
de "el sistema volvio a fallar", en realidad es el rate limiting (ya
confirmado que funciona) bloqueando peticiones legitimas de la propia
suite de pruebas. Esperar unos minutos entre corridas.
"""
...
BASE_URL = "https://sigab-backenddev-jpuya4-ea3a74-158-69-200-27.sslip.io/api-sgpmp"
