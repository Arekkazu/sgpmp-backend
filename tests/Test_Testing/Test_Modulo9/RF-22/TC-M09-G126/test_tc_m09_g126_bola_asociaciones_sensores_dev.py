"""
TC-M09-G126 -- SEGUNDA CORRIDA CONTRA DEV (TC-M09-247, TC-M09-248,
TC-M09-249) - Pruebas de ataque:
...
Re-ejecucion pedida por el usuario (2026-09-13) contra DEV para verificar
si los gaps de multi-tenancy de TEST ya fueron corregidos -- la corrida
original contra TEST (test_tc_m09_g126_bola_asociaciones_sensores.py,
este mismo directorio) se deja intacta como evidencia historica; este es
un archivo nuevo apuntando a DEV. Cuenta Productor distinta a la de TEST
(u20221206763@usco.edu.co no forma parte del roster de DEV) -- se usa
m2m.nuevo@ejemplo.com, ya confirmado en TC-M09-G105/G71 que tambien
posee 0 fincas propias en DEV (mismo perfil: "usuario sin ninguna
relacion con ninguna finca"). id_infraestructura=3 (finca 1,
"Alevinera-01") y =4 (finca 2, "Canal-Trucha-01") confirmados como los
mismos IDs/nombres en DEV.

Como correrlo (desde la raiz del repo):
    python -m pytest <ruta>\\test_tc_m09_g126_bola_asociaciones_sensores_dev.py -v \
        --html=Resultados/reporte-TC-M09-G126-DEV.html --self-contained-html
"""
...
PRODUCTOR_CORREO = "m2m.nuevo@ejemplo.com"
PRODUCTOR_CONTRASENA = "Test1234!"
