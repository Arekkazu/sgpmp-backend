...
definicion previa (con los tres bugs) tal como existia en la base de datos.
"""
...
def upgrade() -> None:
    op.execute(
        ...
        BEGIN
            ...
            -- Validar tipo_agregacion segun tipo de activo
            IF v_tipo_activo = 'INDIVIDUAL' THEN
                IF NEW.tipo_agregacion IS NOT NULL AND TRIM(NEW.tipo_agregacion) <> '' THEN
                    RAISE EXCEPTION 'INVALID_FIELD: Para activos INDIVIDUALES el campo tipo_agregacion debe ser nulo o vacio.'
                    USING ERRCODE = 'P0216';
                END IF;
            ...
            IF NEW.valor_medicion <= 0 THEN
                RAISE EXCEPTION 'INVALID_VALUE: El valor de medicion debe ser positivo y mayor a cero. Valor recibido: %.', NEW.valor_medicion
                USING ERRCODE = 'P0217';
            END IF;

            -- Validar coherencia unidad / tipo de medicion (alineado con
            -- RegistrarEventoCrecimientoDTO._UNIDADES_POR_TIPO)
            IF v_tipo_medicion = 'PESO' AND v_unidad NOT IN ('kg', 'gr', 'lb') THEN
                RAISE EXCEPTION 'UNIT_MISMATCH: Para medicion de PESO solo se permiten unidades: kg, gr, lb. Unidad recibida: %.', NEW.unidad_medida
                USING ERRCODE = 'P0218';
            END IF;

            IF v_tipo_medicion IN ('TALLA', 'LONGITUD', 'ALTURA') AND v_unidad NOT IN ('cm', 'm') THEN
                RAISE EXCEPTION 'UNIT_MISMATCH: Para medicion de TALLA/LONGITUD/ALTURA solo se permiten unidades: cm, m. Unidad recibida: %.', NEW.unidad_medida
                USING ERRCODE = 'P0218';
            END IF;

            IF v_tipo_medicion = 'BIOMASA' AND v_unidad NOT IN ('kg/m2') THEN
                RAISE EXCEPTION 'UNIT_MISMATCH: Para medicion de BIOMASA solo se permite la unidad: kg/m2. Unidad recibida: %.', NEW.unidad_medida
                USING ERRCODE = 'P0218';
            END IF;
...
def downgrade() -> None:
    # Restaura la definicion previa, con los tres bugs descritos arriba.
    op.execute(
        """
        CREATE OR REPLACE FUNCTION modulo2.trg_fn_evento_crecimiento_tipo_activo()
         RETURNS trigger
        ...
        BEGIN
            ...
            IF v_tipo_medicion = 'PESO' AND v_unidad NOT IN ('kg', 'g', 'lb') THEN
                RAISE EXCEPTION 'UNIT_MISMATCH: Para medicion de PESO solo se permiten unidades: kg, g, lb. Unidad recibida: %.', NEW.unidad_medida
                USING ERRCODE = 'P0218';
            END IF;
